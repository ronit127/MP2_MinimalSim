...
public class OrangeTreeStationScript : MonoBehaviour
{
    ...
    public void OnStationClicked()
    {
        if (manager.oranges >= price && upgradeCount < treeLevels.Length)
        {
            ...
            currentProductionText.text = isOwned ? (int)currProductionPower + " oranges per second" : "";
            ...
            {
                statusText.text = isOwned ? "Upgrade: " + (int)price + " oranges" : "Buy: " + (int)price + " oranges";
            }
        }
    }
    ...
    public void ApplyPowerUp(float multiplier)
    {
        ...
        currentProductionText.text = (int)currProductionPower + " oranges per second";
    }
}